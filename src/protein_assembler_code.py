...
parser.add_argument('-m', '--max_chains',
	...
	default =  -1,
	nargs = '+', 
	help = "(integer) Maximum number of subunits for the final model. If limitant_chains is activated, list of maximum limitant chains allowed in final model")
...
parser.add_argument('-f', '--limitant_chains_multiplier',
	dest = "limitant_chains_multiplier",
	action = "store",
	type = int,
	default = 1,
...
if options.limitant_chains != "False":
	...
	if options.unique:
		limitant_ordinals = { chainid : [ 0, chainlimit*options.limitant_chains_multiplier ] for chainid,chainlimit in zip(options.limitant_chains,options.max_chains) }

	#Else, use ordinals as keys
	else: 
		limitant_ordinals = { idchainordinal[chainid] : [ 0, chainlimit*options.limitant_chains_multiplier ] for chainid,chainlimit in zip(options.limitant_chains,options.max_chains) }

#verbose limitant_chains_multiplier
if options.verbose:
